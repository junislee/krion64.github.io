...
Contributing — Mod fields · Component fields · Portraits · Spells · Kits · Items · Known issues · Proficiencies
...
Community builds & telemetry — Browse, load, and merge mod builds published by other players via the Community tab. Publish your own from the bottom toolbar. Anonymized install reports from Infinity Mod Runner feed per-component stability indicators (green/yellow/red). All data is public in the infinity-mod-telemetry repo.
...
i	Yes	Unique mod ID
sv	Yes	Schema version (integer). See Schema versioning below. Current: 1
...
games	No	Engine/game target array derived from tp2 REQUIRE_PREDICATE GAME_IS. Mod-level value is the union of all component games arrays. Omitted = universal (no predicate in tp2). See Game targets (games field) below
...
scn	No	Subcomponent name — resolved WeiDU SUBCOMPONENT @ref. The human-readable group header WeiDU shows during install (e.g. "Improved Golems", "Static PsT Character Portraits"). Siblings (components with the same g) share the same scn. See Subcomponent hierarchy below
...
games	No	Engine/game target array. Auto-derived from tp2 REQUIRE_PREDICATE GAME_IS by scripts/scan_game_targets.py. Omitted = universal (no predicate in the tp2 — assumes any-compat). See Game targets (games field) below
...
A standalone component (the most common case — a checkbox that installs one thing)
A member of a SUBCOMPONENT group — mutually exclusive variants the user picks one of (radio-button group)
A member of a GROUP — a top-level install-order category label (e.g. "Enemy Improvements", "Cosmetic Changes")
...
PRE-TACTICAL TWEAKS
TACTICAL MODS
POST-TACTICAL TWEAKS
NPC CUSTOMIZATION
POST-TACTICAL QUESTS
MUSIC & AUDIO
PORTRAITS
EET FINALIZATION
POST EET
...
Mod metadata, tp2 analysis, and compatibility research also drawn from Gibberlings3, Beamdog Forums, Spellhold Studios, Weaselmods, Artisan's Corner, and Pocket Plane Group.
...
Telemetry infrastructure — New infinity-mod-telemetry repo with weekly GitHub Action aggregation pipeline. Fully transparent — all data is public GitHub Issues
...
Earlier releases (v3.9.0 through v1.0.0) are in CHANGELOG.md.
